...
public class Display {
	...
	private class DisplayAccessor implements DisplayAccess {
		...
		// Andres Navarro
		private void processGameCanvasKeyEvent(	GameCanvas c,
												int k,
												boolean press) {
			...
			// is current
			GameCanvasKeyAccess access = MIDletBridge	.getMIDletAccess()
														.getGameCanvasKeyAccess();
		...
		public void run() {
			...
			if (d != null && d instanceof Alert) {
				...
				if (alert.time != Alert.FOREVER) {
					alert	.getCommandListener()
							.commandAction(	(Command) alert	.getCommands()
															.get(0),
											alert);
				}
			}
	...
	private final class PaintTask implements Runnable {
		...
		public void run() {
			DeviceFactory.getDevice().getDeviceDisplay().repaint(	x,
																	y,
																	width,
																	height);
		}
				...
				try {
					...
					if (runnable instanceof PaintTask) {
						...
						while (eventQueue.peek() != null
								&& eventQueue.peek() instanceof PaintTask) {
	...
	Display() {
		...
		timer.scheduleAtFixedRate(	new RunnableWrapper(new TickerPaintTask()),
									0,
									Ticker.PAINT_TIMEOUT);

		timer.scheduleAtFixedRate(	new RunnableWrapper(new GaugePaintTask()),
									0,
									Ticker.PAINT_TIMEOUT);
	}
	...
	public static Display getDisplay(MIDlet m) {
		...
		if (MIDletBridge.getMIDletAccess(m).getDisplayAccess() == null) {
			...
			result = MIDletBridge	.getMIDletAccess(m)
									.getDisplayAccess()
									.getDisplay();
		}
