...
public class APIDeclarationsTestCase extends TestCase {
	...
	protected boolean failOnce = true;
	...
	protected void verifyClassList(List names, ClassPool implClassPool, ClassPool refClassPool) throws Exception {
		...
		if ((failOnce) && (!reportOnly) && (reportBuffer != null)) {
			fail(reportBuffer.toString());
		}
	}
...
//		}
	...
	public void reportFail(String message) {
		if (reportOnly) {
			...
			reportBuffer.append(message);
		} else {
			fail(message);
		}
	}

	private void reportOrThrow(AssertionFailedError e) {
		if (reportOnly) {
			...
			reportBuffer.append(e.getMessage());
		} else {
			throw e;
		}
	}
