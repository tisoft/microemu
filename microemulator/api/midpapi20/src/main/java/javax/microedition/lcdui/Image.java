...
public class Image {
     ...
     * in <code>0xAARRGGBB</code> format, where the high-order byte contains
     * the alpha channel and the remaining bytes contain color components for
     * red, green and blue, respectively. The alpha channel specifies the
     * opacity of the pixel, where a value of <code>0x00</code> represents a
     * pixel that is fully transparent and a value of <code>0xFF</code>
     * represents a fully opaque pixel.
     * 
     * <p>
     * The returned values are not guaranteed to be identical to values from the
     * original source, such as from <code>createRGBImage</code> or from a PNG
     * image. Color values may be resampled to reflect the display capabilities
     * of the device (for example, red, green or blue pixels may all be
     * represented by the same gray value on a grayscale device). On devices
     * that do not support alpha blending, the alpha value will be
     * <code>0xFF</code> for opaque pixels and <code>0x00</code> for all
     * other pixels (see <a href="#alpha">Alpha Processing</a> for further
     * discussion.) On devices that support alpha blending, alpha channel values
     * may be resampled to reflect the number of levels of semitransparency
     * supported.
     * </p>
     * 
     * <p>
     ...
     *             if the requested operation would attempt to access an element
     *             in the rgbData array whose index is either negative or beyond
     *             its length (the contents of the array are unchanged)
     * @throws IllegalArgumentException -
     *             if the area being retrieved exceeds the bounds of the source
     *             image
     * @throws IllegalArgumentException -
