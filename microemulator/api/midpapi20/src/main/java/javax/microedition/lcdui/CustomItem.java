...
abstract public class CustomItem extends Item {
     ...
     * called by CustomItem subclass code in order for it to determine what
     * kinds of input are available from this device. The modes available may be
     * dependent upon several factors: the hardware keys on the actual device,
     * which of these keys are needed for the system to do proper navigation,
     * the presence of a pointing device, etc. See
     * <a href="#interaction">Interaction Modes</a> for further discussion. If this
     * method returns 0, the only interaction available is through item
     * commands.
     * 
